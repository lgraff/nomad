...
def get_edge_cost_df(G_sn, betas, hour, minute, config):
    ...
    df_edge_costs_sn = costs.edges.static.assign_edge_reliability(df_G, df_tt, hour, minute, config)  # reliability is, for the most part, derived from avg tt
    df_edge_costs_sn = costs.edges.static.assign_edge_price(df_edge_costs_sn, config) # price is derived from avg tt and length
    df_edge_costs_sn = costs.edges.static.assign_edge_risk(df_G, df_edge_costs_sn, config)
    df_edge_costs_sn = costs.edges.static.assign_edge_discomfort(df_edge_costs_sn, config)
