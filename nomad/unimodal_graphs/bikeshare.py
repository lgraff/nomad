""" Module to build bikeshare network graphs"""
...
def build_graph(G_bike, study_area_path, station_filepath, lat_colname, long_colname, id_colname):
    """ Build bikeshare graph from biking graph and bikeshare depot (station) locations"""
    G_bs = G_bike.copy()  
    ...
    # The station nodes are connected to the intersection nodes by 'connection edges' formed by matching the depot to its nearest neighbor in the biking road network
    # Read in bikeshare station locations and build connection edges. Note: at some point, want to generalize this to accept any file type
    df_bs = pd.read_csv(station_filepath)  
    # Generate point geometry from x,y coords, so that the GIS clip function can be used to only include stations within the study region 
