...
def assign_edge_reliability(df_G, df_tt, hr, minute, config):
    ...
    df_rel_ratio = pd.read_csv(data_path['reliability_ratio'])
    ...
    waiting_rel_mult = config['reliability']['BOARDING'] 
    df_boarding['reliability'] = waiting_rel_mult * df_boarding['avg_tt_sec']

    df_pt_trav = df_tt[df_tt.mode_type == 'pt'].reset_index(drop=True)
    df_pt_trav['reliability'] = rel_mult_by_frc[2] * df_pt_trav['avg_tt_sec']    # assume frc = 2 for traversal edges
   ...
   # MICROTRANSIT
    df_mt = df_tt[df_tt.mode_type == 'mt'].reset_index(drop=True)
    df_mt['reliability'] = rel_mult_by_frc[2] * df_mt['avg_tt_sec']    # assume frc = 2 for traversal edges
    ...
    df_twait['reliability'] = config['reliability']['TNC_WAIT'] * df_twait['avg_tt_sec']
    ...
    df_cost = pd.concat([df_boarding, df_pt_trav, df_alight, df_mt, df_tz.drop(columns='frc'), df_twait, df_active], axis=0)
    return df_cost