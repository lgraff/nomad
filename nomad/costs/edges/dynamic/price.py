...
def assign_edge_price(config, df_tt_dynamic):
    ...
    total_fee_arr = fixed_fee_arr + (fee_per_min_arr * tt_arr / 60) + (fee_per_mile_arr * length_arr / config['conversion_factors']['MILE_TO_METERS'])
    ...
    df_price_dynamic.loc[((df_price_dynamic['mode_type'] == 'w') & (df_price_dynamic['target'].str.startswith('sc'))), interval_colnames] = price_params['sc_tx']['fixed']
